...
if (cli.flags.ui) {
    ...
    await withFullScreen(_jsx(TuiApp, { model: cli.flags.model, maxIter: cli.flags.maxIter, enableBash: cli.flags.bash })).start();
}
else if (!prompt && !cli.flags.interactive) {
    cli.showHelp();
}
else {
    const { waitUntilExit } = render(_jsx(App, { prompt: prompt, model: cli.flags.model, maxIter: cli.flags.maxIter, verbose: cli.flags.verbose, enableBash: cli.flags.bash, interactive: cli.flags.interactive }));
    try {
        await waitUntilExit();
    }
    catch {
