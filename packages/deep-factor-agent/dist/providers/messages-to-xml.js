 ...
 */
export function messagesToXml(messages) {
    ...
    for (const msg of messages) {
        ...
        switch (type) {
            ...
            case "ai": {
                ...
                if (aiMsg.tool_calls) {
                    for (const tc of aiMsg.tool_calls) {
                        lines.push(`  <event type="tool_input" id="${id}" name="${escapeXml(tc.name)}" call_id="${escapeXml(tc.id ?? "")}" iteration="0">${escapeXml(JSON.stringify(tc.args))}</event>`);
                        id++;
                    }
                }
                break;
            ...
            case "tool": {
                ...
                lines.push(`  <event type="tool_output" id="${id}" name="${escapeXml(toolName)}" call_id="${escapeXml(toolCallId)}" status="success" iteration="0">${escapeXml(content)}</event>`);
                id++;
                break;
            }
            default:
                lines.push(`  <event type="${escapeXml(type)}" id="${id}" iteration="0">${escapeXml(content)}</event>`);
                id++;
        }
    }
    lines.push("</thread>");
