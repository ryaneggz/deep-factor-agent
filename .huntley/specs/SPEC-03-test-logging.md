...
4. **`logs/` in `.gitignore`**: Log files are ephemeral build artifacts. They should not be committed. The `.huntley/logs/` entry already exists; this adds a root-level `logs/` entry for test output.
